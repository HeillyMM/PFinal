...
<h1>Eliminar Agendación</h1>
<form id="formEliminar" action="agenda_eliminacion2.php" method="post">
<label>ID de la Agendación</label>
<input type="number" name="ida" required>
<input class="btn btn-primary" type="submit" value="Elimine">
</form>
...
document.getElementById("formEliminar").addEventListener("submit", function(e){
    const id = document.querySelector('input[name="ida"]').value;
    if(id.trim() === ""){
        e.preventDefault();
        alert("Debe ingresar un ID");
    } else if(!confirm("¿Desea eliminar esta agendación?")){
        e.preventDefault();
    }
});
</script>
